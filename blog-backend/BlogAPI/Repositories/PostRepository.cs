...
namespace BlogAPI.Repositories
{
    public class PostRepository : BaseRepository
    {
        ...
        {
           return await _context.Posts.Find(Builders<Post>.Filter.Eq(post => post.AuthorId, authorId)
        ...
        {
            return await _context.Posts.Find(Builders<Post>.Filter.Eq(post => post.Category, category)
                                             & Builders<Post>.Filter.Eq(x => x.Status, Status.Aproved)).ToListAsync();
        }
        ...
        {
            return await _context.Posts.Find(Builders<Post>.Filter.Eq(post => post.Status, status)).ToListAsync();
        }
        ...
        public async Task IncrementViewsAsync(string postId)
        {
            ...
            await _context.Posts.UpdateOneAsync(Builders<Post>.Filter.Eq(post => post.Id.ToString(), postId), update);
        }
